 ...
 * @LastEditTime: 2020-08-06 18:23:15
...
void Nv2RGBRender_Gpu::initialize(const int width, const int height, const bool horizontal, const bool vertical)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_BGRA, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE, nullptr);
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER_ARB, width * height * 4, nullptr, GL_STREAM_DRAW_ARB);
...
void Nv2RGBRender_Gpu::render(unsigned char *nv12_dPtr, const int width, const int height)
{
	...
	uint8_t *hostp;
	cudaMalloc(&hostp, d_tex_size);
    Nv12ToColor32<BGRA32>(reinterpret_cast<uint8_t *>(nv12_dPtr), width, hostp, d_tex_size / height, width, height, 5);
	static int index = 0;
	qDebug() << "----------" << index << "src pitch" << d_tex_size / (4 * height) << "d_tex_size" << d_tex_size;
//	ck(cudaMemcpy(file_ptr, reinterpret_cast<void*>(hostp), width* height * 4, cudaMemcpyDeviceToHost));
	ck(cudaMemcpy(file_ptr, reinterpret_cast<void*>(nv12_dPtr), width* height * 3 / 2, cudaMemcpyDeviceToHost));
